        ...
        if (lockTarget)
        {
            if (!(currentState.cameraMode.Equals(TPCameraMode.FixedAngle)))
            {
                ...
                if (collider != null)
                {
                    ...
                    if (isNewTarget)
                    {
                        lookTargetAdjust = Quaternion.LerpUnclamped(lookTargetAdjust, Quaternion.Euler(euler), currentState.smoothFollow * Time.fixedDeltaTime);
                        if (Vector3.Distance(lookTargetAdjust.eulerAngles, euler) < .5f)
                            isNewTarget = false;
                    }
                    else
                        lookTargetAdjust = Quaternion.Euler(euler);
                }
            }
        }
        else
        {
            lookTargetAdjust = Quaternion.LerpUnclamped(lookTargetAdjust, Quaternion.Euler(Vector3.zero), 1 * Time.fixedDeltaTime);
        }
        //rotation.eulerAngles += currentState.rotationOffSet ;
        var _euler = rotation.eulerAngles + lookTargetAdjust.eulerAngles;
        _euler.z = 0;
